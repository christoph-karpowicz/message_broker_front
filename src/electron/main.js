...
function main() {
    ...
    ipcMain.on('addQueue', async (e, data) => {
        const addQueueResponse = await broker.addQueue(data).catch(err => console.error(err));
        e.sender.send('addQueueReply', addQueueResponse);
    });

    ipcMain.on('consume', async (e, data) => {
        const consumeResponse = await broker.consume(data).catch(err => console.error(err));
        e.sender.send('consumeReply', consumeResponse);
    });

    ipcMain.on('length', async (e, data) => {
        const lengthResponse = await broker.getLength(data).catch(err => console.error(err));
        e.sender.send('lengthReply', lengthResponse);
    });

    ipcMain.on('get', async (e, data) => {
        const getResponse = await broker.get(data).catch(err => console.error(err));
        e.sender.send('getReply', getResponse);
    });

    ipcMain.on('getAll', async (e, data) => {
        const getAllResponse = await broker.getAll(data).catch(err => console.error(err));
        e.sender.send('getAllReply', getAllResponse);
    });

    ipcMain.on('getQueueList', async (e) => {
        const getQueueListResponse = await broker.getQueueList().catch(err => console.error(err));
        e.sender.send('getQueueListReply', getQueueListResponse);
    });

    ipcMain.on('produce', async (e, data) => {
        const produceResponse = await broker.produce(data).catch(err => console.error(err));
        e.sender.send('produceReply', produceResponse);
    });
    
    ipcMain.on('removeQueue', async (e, data) => {
        const removeQueueResponse = await broker.removeQueue(data).catch(err => console.error(err));
